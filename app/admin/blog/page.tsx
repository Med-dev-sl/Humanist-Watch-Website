...
export default function AdminBlog() {
  ...
  async function handleSave() {
    ...
    try {
      ...
      setFormOpen(false);
      showToast("success", editing ? "Updated" : "Created", `Post "${form.title}" ${editing ? "updated" : "created"} successfully.`);
      setModal({ type: "success", title: editing ? "Post Updated" : "Post Created", message: `"${form.title}" has been ${editing ? "updated" : "created"}.` });
      fetchPosts();
    } catch (err) {
  ...
  async function handleDelete() {
    ...
    try {
      ...
      showToast("success", "Deleted", `Post "${deleteTarget.title}" deleted.`);
      setModal({ type: "success", title: "Post Deleted", message: `"${deleteTarget.title}" has been removed.` });
      fetchPosts();
    } catch (err) {
  ...
  return (
    <div className="p-8">
      <PageHeader title="Blog Posts" description="Manage blog posts and articles." />
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="flex gap-1 rounded-xl border border-zinc-200 bg-white p-1">
            {(["all", "DRAFT", "PUBLISHED", "ARCHIVED"] as const).map((s) => (
              ...
              >
                {s === "PUBLISHED" ? "Published" : s === "ARCHIVED" ? "Archived" : s === "DRAFT" ? "Draft" : "All"}
              </button>
            ))}
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="py-16 text-center">
            ...
            <p className="mt-1 text-sm text-zinc-400">
              {search || statusFilter !== "all" ? "Try a different search or filter." : "Create your first blog post to get started."}
            </p>
          ...
          <div className="divide-y divide-zinc-100">
            <div className="grid grid-cols-[1fr_130px_160px_100px] gap-4 bg-zinc-50/80 px-6 py-3 text-xs font-semibold uppercase tracking-wider text-zinc-400">
              <span>Title</span>
            ...
            {filtered.map((post, i) => (
              <div
                ...
                className="animate-slide-up grid grid-cols-[1fr_130px_160px_100px] gap-4 px-6 py-4 text-sm transition-colors hover:bg-zinc-50"
              >
                <div className="min-w-0">
                  <p className="truncate font-medium text-primary">{post.title}</p>
                  <p className="truncate text-xs text-zinc-400">/{post.slug}</p>
                </div>
                <div className="flex items-center justify-center">
                  <span className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-medium ${statusColors[post.status]}`}>
                    <span className={`h-1.5 w-1.5 rounded-full ${statusDotColors[post.status]}`} />
            ...
            <div className="flex items-center justify-between border-b bg-gradient-to-r from-primary/[0.03] to-transparent px-6 py-4">
              <div>
                ...
                <p className="text-xs text-zinc-400 mt-0.5">
                  {editing ? "Update the blog post details below." : "Fill in the details to create a new blog post."}
                </p>
              </div>
              <button
            ...
            <div className="space-y-5 px-6 py-6">
              <div className="grid grid-cols-2 gap-5">
                ...
                <div className="col-span-2">
                  <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wider text-zinc-500">Excerpt</label>
                  <textarea
                    rows={3}
                    value={form.excerpt}
            ...
            <div className="flex items-center justify-end gap-3 border-t bg-zinc-50/50 px-6 py-4">
              ...
              >
                {saving || uploading ? (
                  ...
                  <span className="relative z-10">{editing ? "Update Post" : "Create Post"}</span>
                )}
                <span className="absolute inset-0 -translate-x-full bg-gradient-to-r from-transparent via-white/10 to-transparent transition-transform duration-700 group-hover:translate-x-full" />
              </button>
            </div>
        ...
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="animate-scale-in w-full max-w-sm rounded-2xl border bg-white p-8 shadow-2xl mx-4">
            <div className="flex flex-col items-center gap-4 text-center">
              ...
              <div>
                <h2 className="font-heading text-lg font-semibold text-primary">Delete Post</h2>
