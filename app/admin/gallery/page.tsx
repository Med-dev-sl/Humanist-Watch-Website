...
export default function AdminGallery() {
  ...
  useEffect(() => {
    if (formOpen && editing) {
      ...
      setImagePreview(editing.coverImage);
      setImageFile(null);
      setFormErrors({});
  ...
  async function handleSave() {
    ...
    try {
      ...
      setFormOpen(false);
      showToast("success", editing ? "Updated" : "Created", `Gallery item "${form.title}" ${editing ? "updated" : "created"} successfully.`);
      setModal({ type: "success", title: editing ? "Gallery Item Updated" : "Gallery Item Created", message: `"${form.title}" has been ${editing ? "updated" : "created"}.` });
      fetchItems();
    } catch (err) {
  ...
  async function handleDelete() {
    ...
    try {
      ...
      showToast("success", "Deleted", `Gallery item "${deleteTarget.title}" deleted.`);
      setModal({ type: "success", title: "Gallery Item Deleted", message: `"${deleteTarget.title}" has been removed.` });
      fetchItems();
    } catch (err) {
  ...
  async function toggleStatus(item: GalleryItem) {
    try {
      ...
      showToast("success", "Status Updated", `"${item.title}" is now ${!item.published ? "Published" : "Draft"}.`);
      fetchItems();
    } catch {
  ...
  return (
    <div className="p-8">
      <PageHeader title="Gallery" description="Manage photo galleries and albums." />
        ...
        {loading ? (
          ...
          <div className="py-16 text-center">
            ...
            <p className="text-lg font-medium text-primary">
              {search || statusFilter !== "all" ? "No matching items" : "No gallery items yet"}
            </p>
            <p className="mt-1 text-sm text-zinc-400">
              {search || statusFilter !== "all" ? "Try a different search or filter." : "Create your first gallery item to get started."}
            </p>
            ...
            {filtered.map((item, i) => (
              ...
              >
                <div className="min-w-0">
                  ...
                  <p className="truncate text-xs text-zinc-400">/{item.slug}</p>
                </div>
                <div className="flex items-center justify-center">
                  ...
                  >
                    <span className={`h-1.5 w-1.5 rounded-full ${item.published ? "bg-green-500" : "bg-amber-500"}`} />
          ...
          <div className="animate-scale-in relative w-full max-w-2xl rounded-2xl border bg-white shadow-2xl mx-4">
            <div className="flex items-center justify-between border-b bg-gradient-to-r from-primary/[0.03] to-transparent px-6 py-4">
              <div>
                <h2 className="font-heading text-lg font-semibold text-primary">
                  {editing ? "Edit Gallery Item" : "Create Gallery Item"}
                </h2>
                <p className="text-xs text-zinc-400 mt-0.5">
                  {editing ? "Update the gallery item details below." : "Fill in the details to create a new gallery item."}
                </p>
              </div>
              <button
            ...
            <div className="space-y-5 px-6 py-6">
              <div className="grid grid-cols-2 gap-5">
                ...
                <div className="col-span-2">
                  <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wider text-zinc-500">Cover Image</label>
                  <label
                  ...
                  >
                    ...
                    {imagePreview ? (
                      ...
                      <div className="flex flex-col items-center gap-2">
                        ...
                        <span className="text-sm font-medium text-zinc-400 group-hover:text-primary transition-colors">Upload image</span>
            ...
            <div className="flex items-center justify-end gap-3 border-t bg-zinc-50/50 px-6 py-4">
              ...
              >
                {saving || uploading ? (
                  ...
                  <span className="relative z-10">{editing ? "Update Gallery Item" : "Create Gallery Item"}</span>
                )}
                <span className="absolute inset-0 -translate-x-full bg-gradient-to-r from-transparent via-white/10 to-transparent transition-transform duration-700 group-hover:translate-x-full" />
              </button>
            </div>
        ...
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="animate-scale-in w-full max-w-sm rounded-2xl border bg-white p-8 shadow-2xl mx-4">
            <div className="flex flex-col items-center gap-4 text-center">
              ...
              <div>
                <h2 className="font-heading text-lg font-semibold text-primary">Delete Gallery Item</h2>
